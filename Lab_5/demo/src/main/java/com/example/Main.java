...
public class Main {
    ...
    // Sparse graph: chain backbone + a few random edges (~2 edges per node)
    ...
    // Dense graph: every pair of nodes connected (~n*(n-1)/2 edges)
    ...
    static double measurePrim(WeightedGraph graph) {
        int runs = 5;
        long total = 0;
    ...
    static double measureKruskal(WeightedGraph graph) {
        int runs = 5;
        long total = 0;
        for (int i = 0; i < runs; i++) {
            long t0 = System.nanoTime();
    ...
    public static void main(String[] args) {
        ...
        XYSeries primSparse    = new XYSeries("Prim - Sparse");
        XYSeries primDense     = new XYSeries("Prim - Dense");
        XYSeries kruskalSparse = new XYSeries("Kruskal - Sparse");
        XYSeries kruskalDense  = new XYSeries("Kruskal - Dense");
        ...
        for (int n : sizes) {
            ...
            double msSparse = measurePrim(sparse);
            double msDense  = measurePrim(dense);

            primSparse.add(n, msSparse);
            primDense.add(n, msDense);
        ...
        for (int n : sizes) {
            ...
            double msSparse = measureKruskal(sparse);
            double msDense  = measureKruskal(dense);

            kruskalSparse.add(n, msSparse);
            kruskalDense.add(n, msDense);

            System.out.printf("n = %5d  ->  sparse: %.3f ms   dense: %.3f ms%n", n, msSparse, msDense);
        }

        generateImage(primSparse,    primDense,    "Prim");
        generateImage(kruskalSparse, kruskalDense, "Kruskal");
    }
}